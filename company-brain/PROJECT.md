...
| `pending` | Split `/health` so Railway's 30s healthcheck window can't time out. `/health` is now a fast liveness probe (`{status, version}` — no external calls); the original Supabase + Anthropic + Voyage + scheduler probe moves to `/health/detailed`. Tests + DEPLOYMENT.md updated. |
